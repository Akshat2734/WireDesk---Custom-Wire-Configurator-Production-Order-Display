...
def _read_orders(engine, status=None):
...
def _orders_with_failover(status=None):
    try:
        replica = db.engines.get("replica")
        if replica is not None:
            return _read_orders(replica, status)
    except SQLAlchemyError:
        pass
    return _read_orders(db.engine, status)
...
def get_orders():
    ...
    cache_key = f"orders:v1:{status or 'all'}"
    cached = redis_cache.get_json(cache_key)
    if cached is not None:
        return jsonify(cached)

    orders = _orders_with_failover(status)
