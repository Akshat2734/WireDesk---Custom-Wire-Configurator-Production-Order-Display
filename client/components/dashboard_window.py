...
from components.ordersdashboard import OrderCard
...
class OrdersTab(QWidget):
    ...
    def load_orders(self):
        try:
            rows = self.sync_manager.api_client.get_orders(self.status_filter)
        except ApiError:
            rows = []
        for row in rows:
            self.cards_layout.addWidget(OrderCard(row, self.sync_manager))
